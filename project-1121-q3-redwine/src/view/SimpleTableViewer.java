...
public class SimpleTableViewer {
	public static String VIEW_ID = "project-1121-q3-redwine.partdescriptor.simpleviewpatternmatchredwine";
	...
	public void createControls(Composite parent) {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (data instanceof ProgElem) {
					...
					if (dialog.open() == Window.OK) {
						...
						String newName = String.valueOf(dialog.getNewMethodName()).trim();
						if (progElem.getMethod().equalsIgnoreCase(newName)) {
							return;
						}
						try {
							RenameMethodAnalyzer renameAnalyzer = new RenameMethodAnalyzer(progElem, newName);
							renameAnalyzer.analyze();
						} catch (CoreException ex) {
							ex.printStackTrace();
						}
						progElem.setMethod(newName);
						thiz.reset();
					}
				}
			}
		...
		menuItem.setText("Rename Selected Class");
		menuItem.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				TableItem item = viewer.getTable().getSelection()[0];
				...
				if (data instanceof ProgElem) {
					...
					if (dialog.open() == Window.OK) {
						...
						System.out.println("[DBG] CLASS NAME: " + progElem.getClazz());
						System.out.println("[DBG] METHOD NAME: " + progElem.getMethod());
						System.out.println("[DBG] NEW CLASS NAME: " + dialog.getNewMethodName());
						...
						try {
							RenameClassAnalyzer renameAnalyzer = new RenameClassAnalyzer(progElem, newName);
							renameAnalyzer.analyze();
						} catch (CoreException ex) {
							ex.printStackTrace();
						}
